...
public class OnlineService {
    ...
    public void CallbackFunForDataServer(String CallId, byte[] Buf, int Size, int Encode, int Resolution, int IFrame, int TurnType, int Time) {
        ...
        if (!this.VideoInited) {
            Log.i((String)this.tag, (String)("Resolution" + Resolution + "   Encode " + Encode));
    ...
    private void InitPlayer(String CallId, int Encode, int Resolution, int IFrame, int Time) {
        ...
        this.decoderType = Encode == 5 || Encode == 4 ? 2 : (Encode == 1 ? 2 : (Encode == 2 ? 4 : 2));
        this.mEncode = Encode;
        this.mTime = Time;
        this.mCallId = CallId;
        this.mFream = IFrame;

        Log.i((String)this.tag, (String)("decoder——type:  " + this.decoderType));

        if (isDecode_) {
            if(_decoders.init(this.decoderType, 2, this.mFrameWidth, this.mFrameHeight)==-1){
                Log.e(this.tag, "error while init decoder！");
            }
            //Log.d((String)this.tag, (String)("handle   " + _decoders.getHandle()));
    ...
    public String callLanVideo(String devid, int hkid, String DevType, int Channel2, int StreamType) {
        ...
        String mId=this.DoLanInvite(hkid, DevType, Channel2, StreamType);
        Log.e("mId","A"+mId);
                ...
                while (!OnlineService.this.isVideoStop) {
                    ...
                    if (isDecode_) {
                        ...
                        if (OnlineService.this.mFrameWidth != _decoders.frameWidth() || OnlineService.this.mFrameHeight != _decoders.frameHeight()) {
                            ...
                            //Log.d("config*******", (String)("handle    "  +_decoders.getHandle()));

                            Log.d("config*******", (String)("mFrameWidth" + mFrameWidth + "   mFrameHeight " + mFrameHeight));
